...
- **v0.30.1** — Hotfix audit & sécurité : Edge Function `send-push-notification` protégée par garde Bearer SERVICE_ROLE_KEY (sinon endpoint ouvert sur internet = phishing push). CORS Edge Functions Stripe/push : whitelist (avis-base.com + www + SITE_URL + ALLOWED_ORIGINS env), plus de `*`, reflect-origin + `Vary: Origin`. Sitekey Turnstile prod (la test bypassable). Intègre v0.18.2 (RLS server-side 7j + frais Stripe via `balance_transactions` + auto-revoke certif si < 3 articles). Migration `v0.18.2-hotfix-audit-rls-and-cert.sql`.
- **v0.30.0** — Web Push notifications : table `push_subscriptions` + 4 RPCs (subscribe / unsubscribe / list / has), trigger DB `notify_push_on_notification` qui POST vers l'Edge Function `send-push-notification` via `pg_net` à chaque insert dans `notifications`, Edge Function signe VAPID + envoie sur tous les endpoints + supprime les 410 Gone. Service Worker handlers `push` (showNotification) + `notificationclick` (focus existing window ou openWindow). Bouton "🔔 Notifications" dans le menu nav (visible si supporté + configuré) + prompt soft après 3 visites distinctes. Migration `v0.30.0-push-subscriptions.sql` + setup admin (VAPID keys + secrets + pg_net).
- **v0.29.0** — Feed personnalisé : nouvelle étape onboarding "Choisis tes sujets" (chips multi-select min 3, max 10), nouveau mode de feed "Pour toi" (sidebar) qui filtre par sujets aimés + auteurs suivis avec scoring (auteur ×3, sujet ×2, fraîcheur ×0-1), module `UserInterests` qui charge les intérêts à l'auth. Migration `v0.29.0-user-interests.sql` (table `user_interests` + RPCs `set/get_user_interests` + `get_suggested_authors_by_interest`).
...
- **v0.26.0** — Help section clips : modale tutoriel 4 onglets (Capture / Édition / Publication / Best practices) accessible depuis l'éditeur de clips.
